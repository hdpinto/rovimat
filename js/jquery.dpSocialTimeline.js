...
(function ($) {
	...
	SocialTimeline.prototype = {
		init : function(){ 
			...
			if(this.feeds['facebook_page_arr']) {
				for(var i = 0; i < this.feeds['facebook_page_arr'].length; i++) {
					this.feeds['facebook_page_url'][i] = 'http://www.facebook.com/feeds/page.php?format=rss20&id='+$.trim(this.feeds['facebook_page_arr'][i])+'&format=atom10';
				}
		...
		_parseRSS : function(url, limit, int_val, fnk, num){
			...
			if(url.indexOf('user_timeline.php') !== -1 || url.indexOf('search.php') !== -1 || url.indexOf('instagram.php') !== -1 || url.indexOf('instagram_hash.php') !== -1 || url.indexOf('foursquare.php') !== -1) {
				gurl = url;	
